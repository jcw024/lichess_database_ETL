...
def histogram_player_churn(filename):
    ...
    n, bins, patches = plt.hist(df.total_games, bins=200, log=True, histtype='step')
    ax.set_title("Total Games Played per Player Histogram")
    ax.set_xlabel("Number of Games Played")
    ax.set_ylabel("Number of Players")
    plt.xticks(np.arange(0, max(df.total_games)+1, 10000))
    plt.xlim([0,100000])
    plt.savefig(filename, dpi=300, bbox_inches='tight')
