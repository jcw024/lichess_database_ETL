...
with DAG(
    ...
    for i, url in enumerate(url_list):
        ...
        if i == 0:
            download_task >> process_task >> delete_file_task
        else:
            prev_download_task >> download_task
            [prev_process_task, download_task] >> process_task >> delete_file_task
        if i >= 2:
            prev_prev_process_task >> download_task    #downloads are usually faster than producer, we don't want
                                                        #downloaders to far outpace processing of files to minimize disk usage
        if i >= 1:
            prev_prev_process_task = prev_process_task
        prev_download_task = download_task
        prev_process_task = process_task
